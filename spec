...
Если один атакует а второй трусит: выигрывает тот кто атаковал. В этом случае победившему роботу начисляется одна победа, а статус второго робота меняется на "сломан".
Если оба атакуют: производится рандом число от 1 до 6ти. Выигрывает тот чье число больше. Если числа равные, производится повторный бросок. Если и при повторном броске результат равный, обоим присуждается победа и оба становятся сломанными.
...
- самый победоносный робот, да же если он сломан, и его хозяин.
